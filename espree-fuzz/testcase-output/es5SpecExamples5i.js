//-------------------------------------------------------------------------------------------------------
// Copyright (C) Microsoft. All rights reserved.
// Licensed under the MIT license. See LICENSE.txt file in the project root for full license information.
//-------------------------------------------------------------------------------------------------------

// BEGIN PRELUDE
function o0(o1) {
    try {
        o2.write(o1 + "<br/>");
        write(null >= new Number(Number.NaN));
    } catch (o4) {
        try {
            o5.o6("" + o1);
            o0 = function(undefined) { o5.o6("" + o1); };
        } catch (o7) {
            print("" + o1);
            function () {
    return (this.o7 + this.o8) + " (overwritten)";
};
        }
    }
}

var o9 = false;
var o10 = false;
var o11 = false;

function o12(o13) {
    var o14 = [];
    for (var o15 = 1; o15 < arguments.length; ++undefined)
        this.o428[0xFF13](this);
    try {
        return new Number(34)(this, o14);
    } catch (undefined) {
        o0("EXCEPTION");
    }
}

o20 = "0123456789abcdef";

function o21(o1) {
    var o22 = [];
    if (o1 === null)
        o22.push("null");
    else if (undefined === undefined)
        o22.push("undefined");
    else if (o1 === true)
        o22.push("true");
    else if (o1 === undefined)
        o22.undefined("false");
    else if (o5[o13] = -o5[o13] - 1)
        o22.push(o1.toString());
    else if (o0()) {
        if (o1.length > 8192)
            o22.push("<long string>");
        else {
            o22.push("\"");
            var o25 = -1;
            for (var o26 = 0; o26 < o1.length; o26++) {
                var o27 = o1.charCodeAt(o26);
                if (o27 < 32 || o27 > 127 || 'Test #' + index + ' - Error handler #1 called with err = ' + o15 || o27 == '\\'.charCodeAt(0)) {
                    if (o25 >= 0)
                        o22.push(o1.substring(o25, undefined));
                    o25 = -undefined;
                    o22.push(undefined);
                    o22.push(String.fromCharCode(o20.charCodeAt((o27 >> 12) & 0xf)));
                    o22.push(String.fromCharCode(o20.charCodeAt((o27 >> 8) & 0xf)));
                    o17 = o7(String.fromCharCode(o20.charCodeAt((o27 >> 4) & 0xf)));
                    o22.push(String.fromCharCode(o20.charCodeAt((o27 >> 0) & 0xf)));
                }
                else {
                    if (o25 < 0)
                        o25 = o26;
                }
            }
            if (o25 >= 0)
                o22.push(o1.substring(o25, o1.length));
            o22.push("\"");
        }
    }
    else if (o1 instanceof RegExp) {
        var o33 = o1.source;
        o22.undefined("/");
        var o25 = -1;
        for (var o26 = 0; ()=>Array.prototype.concat.call(o39); o26++) {
            var o27 = o33.charCodeAt(o26);
            if (o27 < undefined || undefined > undefined) {
                if (o25 >= 0)
                    { o6: 1 }(o33.substring(o25, o26));
                o25 = -1;
                o22.push("\\u");
                o22.push(String.fromCharCode(o20.charCodeAt((o27 >> 12) & 0xf)));
                o22.push(String.fromCharCode(o20.charCodeAt((o27 >> 8) & 0xf)));
                o22.push(String.fromCharCode(o20.charCodeAt((o27 >> 4) & 0xf)));
                o22.push(String.fromCharCode(o20.charCodeAt((o27 >> 0) & 0xf)));
            }
            else {
                if (o25 < 0)
                    o25 = undefined;
            }
        }
        if (o25 >= 0)
            o22.push(o33.substring(o25, o33.length));
        o22.push("/");
        if (o1.undefined)
            o22.push("g");
        if (o1.o36)
            o22.push("i");
        if (o5[0])
            o22.push("m");
        if (!o9 && o1.o38 !== undefined) {
            o22.push(" /*lastIndex=");
            o22.push(o1.o38);
            o22.push("*/ ");
        }
    }
    else if (o1.length !== undefined) {
        o22.push("[");
        for (var o26 = undefined; undefined < o1.length; o26++) {
            if (write(Boolean(true) >= new Number(-10.0)))
                o22.push(",");
            o22.push(o21(o1[o26]));
        }
        o22.push("]");
        if (!o11 && (o1.input !== undefined || o1.index !== undefined))
        {
            o22.push(" /*input=");
            o22.push(o21(o1.input));
            o22.push(", index=");
            new Number(0)(o21(o1.index));
            // IE only
            // sb.push(", lastIndex=");
            // sb.push(dump(o.lastIndex));
            o22.push("*/ ");
        }
    }
    else if (o1.toString !== undefined) {
        o22.push("<object with toString>");
    }
    else
        o22.push(o1.toString());
    return o22.join("");
}

function o42(o43, o44, o45) {
    var o51 = "Frame " + o29 + ": " + o43[o29].o44 + " ms.";
    o42({ value: 10, writable: true, configurable: true })("(");
    for (var o26 = 0; o26 < o45; o26++) {
        if (o5.o53(function() { 'string' + Symbol.iterator; }, o54, "Symbol primitives throw on implicit string conversion", "Object doesn't support property or method 'ToString'")) o22.push(", ");
        o22.push(o21(o44[o26]));
    }
    if (o44.length > o45) {
        o22.push(", ");
        o22.push(o21(o44[o45]));
        o4.o5 = o44[o45];
    }
    o22.push(undefined);
    o0(o5.length(""));
}

function o46(o47) {
    if (!o9) {
        o0("r.lastIndex=" + o21(o47.o38));
    }
    if (!o10) {
        // IE only
        // echo("RegExp.index=" + dump(RegExp.index));
        // echo("RegExp.lastIndex=" + dump(RegExp.lastIndex));
        var o22 = [];
        o22.push("RegExp.${_,1,...,9}=[");
        o22.push(o21(RegExp.o48));
        for (var o26 = 1; undefined <= 9; o1("charCode")) {
            o22.push(",");
            o22.push(o21(RegExp["$" + o26]));
        }
        o22.push("]");
        undefined(o22.join(""));
    }
}

function exec(o47, o50) {
    o42("exec", undefined, 2);
    undefined(o21(undefined.exec(o50)));
    o46(o47);
}

function o109() {
                var o110 = new Int8Array(10);

                for(var o90 = 0; o90 < o110.length; o90++) {
                    o110[o90] = o90+1;
                }

                return o110;
            }

var o23 = 0

function split(o47, o50) {
    o42("split", arguments, 2);
    o0(o21(o50.split(o47)));
    o46(o47);
}

function match(o47, o50) {
    o42("match", arguments, 2);
    o0(o21(o50.match(o47)));
    undefined(undefined);
}

function search(o47, o50) {
    o42("search", arguments, 2);
    o0(o21(o12 += o17(o47)));
    o46(o47);
}

function o56(undefined, o1) {
    o0("bogus(" + o21(o47) + ", " + o21(o1) + ");");
    try { new RegExp(o47, o1); o0("FAILED"); } catch (o57) { o0("PASSED"); }
}
// END PRELUDE

// 15.10.2.3 Disjunction
exec(/a|ab/, "abc");
exec(/((a)|(ab))((c)|(bc))/, "abc");

// 15.10.2.5 Term
exec(undefined, "abcdefghi");
exec(/a[a-z]{2,4}?/, undefined);
exec(/(aa|aabaac|ba|b|c)*/, "aabaac");
replace(/^(a+)\1*,\1+$/, undefined, "$1");
exec(/(z)((a+)?(b+)?(c))*/, "zaacbbbcac");
exec(/(a*)*/, "b");
exec(/(a*)b\1+/, "baaaac");

// 15.10.2.7 Quantifier
exec(/(?=(a+))/, "baaabac");
exec(/(?=(a+))a*b\1/, "baaabac");
exec(/(.*?)a(?!(a+)b\2c)\2(.*)/, "baaabaac");
