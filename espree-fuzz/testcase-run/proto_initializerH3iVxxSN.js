//-------------------------------------------------------------------------------------------------------
// Copyright (C) Microsoft. All rights reserved.
// Licensed under the MIT license. See LICENSE.txt file in the project root for full license information.
//-------------------------------------------------------------------------------------------------------

/// <reference path="protolib.js" />
try {
if (this.Uint8Array.hasOwnProperty(Symbol.species) && this.o0.o1) {
    try {
o0.o1("protolib.js");
}catch(e){}
}
}catch(e){}

// Test __proto__ (object literal) initializer in a new engine:
//      Run "enabled" function, disable__proto__, then run "disabled" (or by default "enabled" again).
function o534(o3, /*optional*/o569) {
    var o510 = o474.o603(o31, {
                o682: !o751
            });

    try {
try { {
o4.o11(function () { try {
Array.from(o360.prototype.o591, 'string');
}catch(e){} }, Error, "Calling Array.from with non-object mapFn argument throws TypeError", "Array.from: argument is not a Function object");
} } catch(e) {}try { {
o4.o5(Object(o31) === Object(o31), "Two different auto-boxed symbols of the same symbol are never strict-equal to each other");
} } catch(e) {}
}catch(e){}
    try {
o5.o8(o6.o7);
}catch(e){}
    try {
o5.o7(o4 || o3);
}catch(e){}
}

var o582 = o421.o143 + (o421.o148 & 0xFF);
try {
o28.o7(o9);
}catch(e){}

// Used by: Verify not accidentally share code with global InitFld
var o2 = false
